...
"""
convMinusculas = lambda palabra: palabra.lower()
resultadoPalabra = convMinusculas("VICO C - PREGUNTALE A TU Papa POR MI")

print(resultadoPalabra)
...
def funcion3p(a,b,accion):
        if accion=="suma":
            return a+b
        elif accion=="resta":
            return a-b
        else:
            raise Exception("Suma o resta solo admite dos modos, suma o resta")

print(funcion3p(2,3,"restadd"))
...
def funcInvertir(frase):
    palabras_lista = frase.split(" ")
    return " ".join(palabras_lista[::-1])

print(funcInvertir("hola señores y señoras"))
...
def listaobtener(frase):
    for letra in frase:
        if letra != " ":
            list_palabra.append(letra.lower())
    return list_palabra

def palindromo(frase):
    fraseobtenida = listaobtener(frase)
    if(fraseobtenida == fraseobtenida[::-1]):
        return True
    else:
        return False
        
print(palindromo("sOS"))
...
print(convLista(lista_nesteada))
