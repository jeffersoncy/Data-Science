...
class Taxi(Vehiculo):
    ...
    def cobrar(self):
        print("Cantidad a cobrar = {} Euros".format(self.distancia_recorrida*self.tarifa))
    
    def añadir_distancia(self, distancia):
        self.distancia_recorrida += distancia
        
    def añadir_tiempo(self, horas):
        self.añadir_distancia(horas*self.velocidad)
        
taxi = Taxi("Twingo", 80)
taxi.describir_estado()
taxi.cobrar()
taxi.añadir_distancia(30)
taxi.cobrar()
taxi.describir_estado()
taxi.describir()
taxi.acelerar(80)
...
class Parking():
    ...
    def __init__(self, LimVehiculos, VehiculosAparcados):
        self.LimVehiculos = LimVehiculos
        self.VehiculosAparcados = VehiculosAparcados
        self.ListaVehiculos = []
    
    def aparcar(self,Carro):
        if (type(Carro) == Vehiculo):
            if(self.VehiculosAparcados < self.LimVehiculos):
                self.ListaVehiculos.append(Carro)
                print(self.ListaVehiculos)
                self.VehiculosAparcados += 1;
            else:
                print("El parqueadero no permite más vehiculos, espere a una nueva plaza")
        else:
            print("este no es un vehiculo")
    
    def describir(self):
        print("Nro vehiculos ",self.VehiculosAparcados)
        
    def ValidarVehiculo(self,Carro):
        if Carro in self.ListaVehiculos:
            return True
        return False
    
    def sacar_vehiculo(self, vehiculo):
        if vehiculo in self.ListaVehiculos:
            print("Sacando el vehiculo {}".format(vehiculo))
            self.ListaVehiculos.pop(self.ListaVehiculos.index(vehiculo))
        else:
            print("Vehiculo {} no está aparcado".format(vehiculo))
    
    
parking = Parking(12, 10)
parking.describir()
parking.aparcar(coche_manuel)
parking.describir()
print(parking.ValidarVehiculo(taxi))
print(parking.ValidarVehiculo(coche_manuel))
parking.sacar_vehiculo(coche_manuel)